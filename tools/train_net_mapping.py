...
from fcn.train import train_mapping_net
...
def parse_args():
    ...
    parser.add_argument('--pretrained_ref', dest='pretrained_ref',
                        help='initialize with pretrained checkpoint',
                        default=None, type=str)
    ...
    parser.add_argument('--network', dest='network_name',
                        ...
                        default=None, type=str)
    parser.add_argument('--network_ref', dest='network_name_ref',
                        help='name of the network',
                        default=None, type=str)
...
if __name__ == '__main__':
    ...
    # prepare network ref
    assert(args.pretrained_ref)
    if args.pretrained_ref:
        network_data_ref = torch.load(args.pretrained_ref)
        if isinstance(network_data_ref, dict) and 'model' in network_data_ref:
            network_data_ref = network_data_ref['model']
        print("=> using pre-trained network '{}'".format(args.network_name_ref))
    else:
        network_data_ref = None
        print('no reference network checkpoint provided')

    network_ref = networks.__dict__[args.network_name_ref](dataset.num_classes, cfg.TRAIN.NUM_UNITS, network_data_ref).cuda()
    network_ref = torch.nn.DataParallel(network_ref).cuda()
    ...
    for epoch in range(args.startepoch, args.epochs):
        ...
        train_mapping_net(dataloader, background_loader, network, network_ref, optimizer, epoch)
