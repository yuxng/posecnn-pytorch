...
declare -a object=("orange_drill" "toy_plane" "pouch" "tomato_soup" "vim_mug" "black_drill" "blue_mug" "cheezit" "duplo_dude" "duster" "graphics_card" "oatmeal_crumble" "remote" "rinse_aid" "vegemite" "vegemite2")

declare -a set="evaluation"

image_a_path="/capri/HOME_DATASET/fork_00/000000-color.png"
image_b_path="/capri/HOME_DATASET/fork_01/000000-color.png"
for j in "${object[@]}"
do
...
time ./tools/test_cosegmentation.py --gpu $1 \
  ...
  --pretrained_cor $outdir1/seg_resnet34_8s_contrastive_euclidean_epoch_$2.checkpoint.pth  \
  --dataset shapenet_object_test \
  --cfg experiments/cfgs/seg_resnet34_8s_embedding.yml \
  --cfg1 experiments/cfgs/seg_resnet34_8s_contrastive_euclidean.yml
