...
from fcn.test_common import _vis_minibatch_autoencoder, _vis_minibatch_cosegmentation, _vis_minibatch_triplet
from fcn.test_common import _vis_minibatch_segmentation, _vis_minibatch_correspondences, _vis_minibatch_prototype
from fcn.test_common import _vis_minibatch_mapping
...
#************************************#
# train deep co-segmentation network #
#************************************#

def train_docsnet(train_loader, background_loader, network, optimizer, epoch,
    contrastive=False, contrastive_pixelwise=False, prototype=False):
    ...
    for i, sample in enumerate(train_loader):
        ...
        affine_a = affine[:, 0].contiguous().cuda()
        affine_b = affine[:, 1].contiguous().cuda()
        ...
        for j in range(2):
            ...
            if j == 0:
                # affine transformation of background
                if cfg.TRAIN.SYN_BACKGROUND_AFFINE:
                    grids_a = nn.functional.affine_grid(affine_a, image_a.size())
                    background_a = nn.functional.grid_sample(background_color, grids_a, padding_mode='border')
                    input_a = mask_a * image_a + (1 - mask_a) * background_a[:num, :, :h, :w]
                else:
                    input_a = mask_a * image_a + (1 - mask_a) * background_color[:num, :, :h, :w]
            else:
                if cfg.TRAIN.SYN_BACKGROUND_AFFINE:
                    grids_b = nn.functional.affine_grid(affine_b, image_b.size())
                    background_b = nn.functional.grid_sample(background_color, grids_b, padding_mode='border')
                    input_b = mask_b * image_b + (1 - mask_b) * background_b[:num]
                else:
        ...
        if contrastive_pixelwise:
            ...
            loss_matches, loss_masked_non_matches, loss_others_non_matches, loss_background_non_matches, features_a, features_b \
                = network(input_a, input_b, label_a, label_b, matches_a, matches_b, masked_non_matches_a, masked_non_matches_b, \
                          others_non_matches_a, others_non_matches_b, background_non_matches_a, background_non_matches_b, flag)

            loss_match = cfg.TRAIN.EMBEDDING_LOSS_WEIGHT_MATCH * loss_matches.sum()
            loss_masked_non_match = cfg.TRAIN.EMBEDDING_LOSS_WEIGHT_NONMATCH * loss_masked_non_matches.sum()
            loss_others_non_match = cfg.TRAIN.EMBEDDING_LOSS_WEIGHT_NONMATCH * loss_others_non_matches.sum()
            loss_background_non_match = cfg.TRAIN.EMBEDDING_LOSS_WEIGHT_BACKGROUND * loss_background_non_matches.sum()
            loss = loss_match + loss_masked_non_match + loss_others_non_match + loss_background_non_match

            print('[%d/%d][%d/%d], loss %.4f, l_match %.4f, l_non_mask %.4f, l_non_other %.4f, l_non_background %.4f, lr %.6f, time %.2f' \
               % (epoch, cfg.epochs, i, epoch_size, loss, loss_match, loss_masked_non_match, loss_others_non_match, loss_background_non_match, \
                  optimizer.param_groups[0]['lr'], batch_time.val))

        elif contrastive:
            loss_matches, loss_non_matches, features_a, features_b \
                = network(input_a, input_b, label_a, label_b, matches_a, matches_b, masked_non_matches_a, masked_non_matches_b, \
                          others_non_matches_a, others_non_matches_b, background_non_matches_a, background_non_matches_b, flag)

            loss_match = cfg.TRAIN.EMBEDDING_LOSS_WEIGHT_MATCH * loss_matches.sum()
            loss_non_match = cfg.TRAIN.EMBEDDING_LOSS_WEIGHT_NONMATCH * loss_non_matches.sum()
            loss = loss_match + loss_non_match

            print('[%d/%d][%d/%d], loss %.4f, l_match %.4f, l_non_match %.4f, lr %.6f, time %.2f' \
               % (epoch, cfg.epochs, i, epoch_size, loss, loss_match, loss_non_match, optimizer.param_groups[0]['lr'], batch_time.val))

        elif prototype:

            loss, intra_cluster_loss, inter_cluster_loss, features_a, features_b = network(input_a, input_b, 
                label_a, label_b, matches_a, matches_b, masked_non_matches_a, masked_non_matches_b, others_non_matches_a, others_non_matches_b,
                background_non_matches_a, background_non_matches_b, flag)
            loss = torch.sum(loss)
            ...
            out_logsoftmax_a, out_logsoftmax_b, out_weight_a, out_weight_b, out_label_a, out_label_b = network(input_a, input_b, 
                label_a, label_b, matches_a, matches_b, masked_non_matches_a, masked_non_matches_b, others_non_matches_a, others_non_matches_b,
                background_non_matches_a, background_non_matches_b, flag)

            loss_a = loss_cross_entropy(out_logsoftmax_a, out_weight_a)
            loss_b = loss_cross_entropy(out_logsoftmax_b, out_weight_b)
            loss = loss_a + loss_b

            print('[%d/%d][%d/%d], loss_a %.4f, loss_b %.4f, loss %.4f, lr %.6f, time %.2f' \
               % (epoch, cfg.epochs, i, epoch_size, loss_a, loss_b, loss, \
                  optimizer.param_groups[0]['lr'], batch_time.val))

        if cfg.TRAIN.VISUALIZE:
            if contrastive_pixelwise:
                _vis_minibatch_correspondences(input_a, input_b, label_a, label_b, features_a, features_b, background_color, matches_a, matches_b,
                    masked_non_matches_a, masked_non_matches_b, others_non_matches_a, others_non_matches_b,
                    background_non_matches_a, background_non_matches_b)
            elif prototype or contrastive:
                _vis_minibatch_prototype(input_a, input_b, label_a, label_b, features_a, features_b, background_color, flag=flag)
            else:
...
#************************************#
def train_triplet_net(train_loader, background_loader, network, optimizer, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        label_positive = sample['label_positive'].cuda()
        label_negative = sample['label_negative'].cuda()

        if cfg.TRAIN.VISUALIZE:
            _vis_minibatch_triplet(image_anchor, image_positive, image_negative)

        loss_matches, loss_non_matches, features_anchor, features_positive, features_negative \
            = network(image_anchor, image_positive, image_negative, label_positive, label_negative)
...
#************************************#
def train_mapping_net(train_loader, background_loader, network, network_ref, optimizer, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        if cfg.TRAIN.VISUALIZE:
            _vis_minibatch_mapping(inputs, image_target)

        # compute target features
        features_target = network_ref(image_target).detach()

        # foward pass
        loss_matches, features = network(inputs, features_target)
        loss = cfg.TRAIN.EMBEDDING_LOSS_WEIGHT_MATCH * loss_matches.sum()
...
#************************************#
# train deep segmentation network    #
#************************************#
...
def train_segnet(train_loader, background_loader, network, optimizer, epoch, embedding=False, rrn=False):
    ...
    for i, sample in enumerate(train_loader):
        ...
        if embedding:
            loss, intra_cluster_loss, inter_cluster_loss, features = network(inputs, label)
            loss = torch.sum(loss)
            intra_cluster_loss = torch.sum(intra_cluster_loss)
            inter_cluster_loss = torch.sum(inter_cluster_loss)
            out_label = None
            ...
            out_logsoftmax, out_weight, out_label = network(inputs, label)
            loss = loss_cross_entropy(out_logsoftmax, out_weight)
            features = None

        if cfg.TRAIN.VISUALIZE:
            _vis_minibatch_segmentation(inputs, label, background_color, out_label, features)
