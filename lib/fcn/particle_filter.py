...
class particle_filter():
    def __init__(self, cfg_pf, n_particles=100, resample_method='systematic'):
        ...
        self.uv[:, 2] = 1
        self.z = np.zeros((n_particles, ), dtype=float)
        ...
        self.rot_filter = nn.Conv3d(1, 1, cfg_pf.ROT_GAUSSIAN_KERNEL_SZ, bias=False)
    ...
    def add_noise_rot(self):
        ...
        rot_pad = self.rot[uni_v].detach().repeat(1, 1, 1, 3, 3)[:, :, :, (72 - self.cfg_pf.ROT_GAUSSIAN_KERNEL_SZ//2):(72 * 2 + self.cfg_pf.ROT_GAUSSIAN_KERNEL_SZ//2), (72 - self.cfg_pf.ROT_GAUSSIAN_KERNEL_SZ//2):(72 * 2 + self.cfg_pf.ROT_GAUSSIAN_KERNEL_SZ//2)]
        rot_pad = self.ele_pad(rot_pad).detach()
        rot_filtered = self.rot_filter(rot_pad)
    ...
    def resample_ddpf(self, codepose, intrinsics, cfg_pf):
        ...
        self.uv_bar = np.mean(self.uv, axis=0)
        self.z_bar = np.mean(self.z, axis=0)
        self.trans_bar = back_project(self.uv_bar, intrinsics, self.z_bar)

        if len(self.trans_bar.shape) > 1:
            self.trans_bar = self.trans_bar.squeeze(1)
        ...
        rot_wt = rot_wt.detach().cpu().numpy()
