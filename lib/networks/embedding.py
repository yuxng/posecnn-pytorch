...
def spherical_mean(x, cluster_masks, K):
...
class EmbeddingLoss(nn.Module):
    ...
    def forward(self, x, cluster_masks):
        """ Compute the clustering loss. Assumes the batch is a sequence of consecutive frames
            ...
            @param x: a [batch_size x C x H x W] torch.FloatTensor of N NORMALIZED pixel embeddings
        ...
        # Compute cluster spherical means across batch dimension
        cluster_means = spherical_mean(x, cluster_masks, K) # Shape: [batch_size x C x K]
        ...
        intra_cluster_distances = 0.5 * (1 - torch.sum(x * tiled_cluster_means, dim=1)) # Shape: [batch_size x H x W]
        ...
        ### Inter cluster loss ###
        if K > 1:
            inter_cluster_distances = .5 * (1 - torch.sum(cluster_means.unsqueeze(2) * cluster_means.unsqueeze(3), dim=1)) # Shape: [batch_size x K x K]
            ...
            inter_cluster_loss = torch.sum(torch.zeros(1, device=x.device))
