...
class NVObject(data.Dataset, datasets.imdb):
    def __init__(self, image_set, nv_object_path = None):

        self._name = 'nv_object_' + image_set
        self._image_set = image_set
        self._nv_object_path = self._get_default_path() if nv_object_path is None \
                            else nv_object_path
        self._model_path = os.path.join(datasets.ROOT_DIR, 'data', 'models')

        # define all the classes
        self._classes_all = ('__background__', 'AlphabetSoup', 'BBQSauce', 'Butter', 'Cherries', 'ChocolatePudding', 
        ...
        # 3D model paths
        self.model_sdf_paths = ['{}/{}/google_16k/textured_simple.pth'.format(self._model_path, cls) for cls in self._classes_all[1:]]
        ...
        for cls in self._classes_all[1:]:
            filename = '{}/{}/google_16k/textured_simple.ply'.format(self._model_path, cls)
            if osp.exists(filename):
                self.model_mesh_paths.append(filename)
                continue
            filename = '{}/{}/google_16k/textured_simple.obj'.format(self._model_path, cls)
            if osp.exists(filename):
                self.model_mesh_paths.append(filename)
                continue
        ...
        for cls in self._classes_all[1:]:
            filename = '{}/{}/google_16k/texture_map.png'.format(self._model_path, cls)
            if osp.exists(filename):
    ...
    def _get_default_path(self):
        """
        Return the default path where nv_object is expected to be installed.
        """
        return os.path.join(datasets.ROOT_DIR, 'data', 'NV_Object')
    ...
    def _load_object_points(self, classes, extents, symmetry):
        ...
        for i in range(1, num_classes):
            point_file = os.path.join(self._model_path, classes[i], 'google_16k', 'points.xyz')
            print(point_file)
