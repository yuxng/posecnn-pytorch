...
class MOPEDEncoder(data.Dataset, datasets.imdb):

    def __init__(self, image_set, moped_object_path = None):

        self._name = 'moped_encoder_' + image_set
        self._image_set = image_set
        self._moped_object_path = self._get_default_path() if moped_object_path is None \
                            else moped_object_path
        self.root_path = self._moped_object_path

        # define all the classes
        self._classes_all = ('__background__', 'black_drill', 'duplo_dude', 'graphics_card', 'oatmeal_crumble', 'pouch', \
        ...
        # 3D model paths
        self.model_sdf_paths = ['{}/models/{}/reference/integrated_registered_processed.pth'.format(self._moped_object_path, cls) for cls in self._classes_all[1:]]
        ...
        for cls in self._classes_all[1:]:
            filename = '{}/models/{}/reference/integrated_registered_processed.ply'.format(self._moped_object_path, cls)
            if os.path.exists(filename):
                self.model_mesh_paths.append(filename)
                continue
            filename = '{}/models/{}/reference/integrated_registered_processed.obj'.format(self._moped_object_path, cls)
            if os.path.exists(filename):
                self.model_mesh_paths.append(filename)
                continue
        ...
        for cls in self._classes_all[1:]:
            filename = '{}/models/{}/reference/texture_map.png'.format(self._moped_object_path, cls)
            if os.path.exists(filename):
        ...
        if cfg.MODE == 'TEST' and cfg.TEST.BUILD_CODEBOOK:
            filename = os.path.join(self._moped_object_path, 'pose_list.pth')
            poses_rbpf = torch.load(filename)
            self.poses_rbpf = poses_rbpf.cpu().numpy()
            self._size = self.poses_rbpf.shape[0]

        assert os.path.exists(self._moped_object_path), \
                'moped_object path does not exist: {}'.format(self._moped_object_path)
    ...
    def _get_default_path(self):
        """
        Return the default path where moped_object is expected to be installed.
        """
        return os.path.join(datasets.ROOT_DIR, 'data', 'MOPED')


    def _load_object_extents(self):

        extent_file = os.path.join(self._moped_object_path, 'extents.txt')
    ...
    def _load_object_points(self):
        ...
        for i in xrange(len(self._classes)):
            point_file = os.path.join(self._moped_object_path, 'models', self._classes[i], 'reference', 'points.xyz')
            print(point_file)
