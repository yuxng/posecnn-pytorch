...
if __name__ == '__main__':
    ...
    for i in range(len(classes)):
        ...
        # list images
        dirname = os.path.join(model_path, 'data', cls, '*.jpg')
        ...
        for j in range(len(files)):
            ...
            filename = os.path.join(model_path, 'data', cls, 'masks', name[:-4] + '_mask.pbm')
            ...
            if cls == '040_large_marker':
                ...
                dy = nz_y - h/2
                dx = nz_x - w/2
                index = np.where((np.absolute(dy) > threshold) | (np.absolute(dx) > threshold))[0]
                mask[nz_y[index], nz_x[index]] = 255

            # process mask
            nz_y, nz_x = np.where(mask == 0)
            start_x = np.min(nz_x)
            end_x = np.max(nz_x)
            start_y = np.min(nz_y) - 20
            end_y = np.max(nz_y) + 20
            ...
            filename = os.path.join(outdir, name[:-4] + '_mask.pbm')
